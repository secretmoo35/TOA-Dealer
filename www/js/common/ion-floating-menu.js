    ...
    function ionFloatingMenu() {
        return {
            ...
            template: '<ul id="floating-menu" ng-style="{\'bottom\' : \'{{bottom}}\'}" ng-class="{\'active\' : isOpen}" ng-click="open()">' +
                    ...
                    '<span><li class="menu-button icon menu-icon" ng-class="icon" ng-style="{\'background-color\' : buttonColor, \'color\': iconColor}"><span class="badge badge-assertive" ng-if="badge > 0">{{badge}}</span></li></span>' +
                    '</ul>',
            replace: false,
            transclude: true,
